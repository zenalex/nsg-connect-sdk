...
void main() {
  ...
  test(
    ...
    () async {
      ...
      h.controller.init();
      // **issue #114**: тут был голый `pumpEventQueue()` — он прокачивает
      // микротаски фиксированное число раз, а `init()` идёт к кэшу через
      // несколько асинхронных прыжков (открытие БД, запрос, декод строк).
      // В одиночку успевало, под нагрузкой полного сьюта — нет, и тест
      // «моргал» раз в несколько прогонов. Соседний тест ниже вылечен от
      // того же ещё 22.07; здесь правку не применили.
      //
      // Ждём УСЛОВИЕ, а не «сколько-то тиков»: увеличенная задержка прячет
      // флак, а ожидание условия его убирает. Сеть при этом всё так же
      // висит — сам смысл теста (Ready ДО ответа сети) не размывается.
