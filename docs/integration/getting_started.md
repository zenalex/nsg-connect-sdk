...
- `showPendingAnnouncements(context, productExternalKey: ..., onOpenRoute: ...)` — in-app announcements shown on entry (planned maintenance, release notes, a nudge to open a screen or an external page). The server decides **what** to show — expiry, per-user "already seen", product targeting; do not re-implement those rules on your side. Three kinds: `text` (read and close), `route` (a button into your app), `link` (a button to an external URL). `onOpenRoute` receives a product-defined route and an opaque payload; omit it and route-kind announcements render as plain text instead of showing a button that goes nowhere. **`link` needs nothing from you** — an external address means the same thing in every app, so the SDK opens it itself via `url_launcher` (http/https only). Announcement bodies are markdown, the same subset chat bubbles use. Errors are swallowed: failing to show an announcement is bad, breaking app start is worse. See [tasks/TASK91.md](../tasks/TASK91.md).
